    ...
    async (fileKey: string) => {
      ...
      try {
        ...
        const headers: string[] = rawHeaderValues
          .map((value: ExcelJS.CellValue | undefined): string => {
            if (value instanceof Date) {
              ...
              if (
                "text" in value &&
                typeof (value as { text: any }).text === "string"
              ) {
                return (value as { text: string }).text;
              }
              if ("result" in value) {
                return (value as { result: any }).result?.toString() ?? "";
              }
              return String(value);
          ...
          if (rowNumber > 1) {
            ...
            headers.forEach((headerName, index) => {
              ...
              if (cell.value instanceof Date) {
                ...
                rowData[headerName] =
                  (cell.value as { text: any }).text?.toString() ?? null;
              } else {
                rowData[headerName] = cell.value?.toString() ?? null;
              }
            });
        ...
        if (jsonData.length === 0) {
          showToast.error("No data found in the Excel file after headers.");
          setActiveTable(null);
          return;
        }
        setActiveTable({
    ...
    async (fileKey: string) => {
      ...
      try {
        ...
        if (fileKey === "facultyMatrix") {
          ...
          const facultyMatrixResponse = response as FacultyMatrixUploadResult;
          ...
          if (
            ...
            facultyMatrixResponse.flaskWarnings.forEach((warning) => {
