...
export const useFeedbackQuestionsByFormId = (formId: IdType) => {
  return useQuery<FeedbackQuestion[], Error>({
    ...
    queryFn: () => feedbackQuestionService.getFeedbackQuestionsByFormId(formId),
    enabled: !!formId,
  });
};
...
export const useUpdateQuestionCategory = () => {
  ...
  >({
    ...
    onSuccess: (updatedCategory) => {
      ...
      queryClient.invalidateQueries({
        queryKey: QUESTION_CATEGORY_QUERY_KEYS.detail(updatedCategory.id),
      });
...
export const useSoftDeleteQuestionCategory = () => {
  ...
  return useMutation<
    ...
    { previousCategoriesList: QuestionCategory[] | undefined }
  >({
    mutationFn: (id) => feedbackQuestionService.softDeleteQuestionCategory(id),
    onSuccess: (_, id) => {
      queryClient.invalidateQueries({
        queryKey: QUESTION_CATEGORY_QUERY_KEYS.lists(),
      });
...
export const useCreateFeedbackQuestion = () => {
  ...
  >({
    mutationFn: ({ formId, questionData }) =>
      feedbackQuestionService.createFeedbackQuestion(formId, questionData),
    onSuccess: (newQuestion) => {
      // Invalidate the questions list for the specific form
      queryClient.invalidateQueries({
        queryKey: FEEDBACK_QUESTION_QUERY_KEYS.listsByForm(newQuestion.formId),
      });
...
export const useSoftDeleteFeedbackQuestion = () => {
  ...
  >({
    mutationFn: (id) => feedbackQuestionService.softDeleteFeedbackQuestion(id),
    onSuccess: (_, id) => {
      queryClient.invalidateQueries({
    ...
    onSettled: (_data, _error, _idToDelete, context) => {
      if (context?.formId) {
        queryClient.invalidateQueries({
          queryKey: FEEDBACK_QUESTION_QUERY_KEYS.listsByForm(context.formId),
        });
        queryClient.invalidateQueries({
...
export const useUpdateFeedbackQuestion = () => {
  ...
  >({
    ...
    onSuccess: (updatedQuestion) => {
      queryClient.invalidateQueries({
        queryKey: FEEDBACK_QUESTION_QUERY_KEYS.detail(updatedQuestion.id),
      });
      queryClient.invalidateQueries({
    ...
    onMutate: async ({ id, data: updateData }) => {
      ...
      if (formId) {
        queryClient.setQueryData<FeedbackQuestion[]>(
          FEEDBACK_QUESTION_QUERY_KEYS.listsByForm(formId),
          (old) => old?.map((q) => (q.id === id ? { ...q, ...updateData } : q)),
        );
      }
    ...
    onError: (err, variables, context) => {
      ...
      if (context?.previousQuestionsList && context.previousQuestion?.formId) {
        queryClient.setQueryData(
