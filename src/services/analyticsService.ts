...
const analyticsService = {
  ...
  getAllFacultyPerformanceData: async (
    ...
    try {
      ...
      >(ANALYTICS_ENDPOINTS.GET_ALL_FACULTY_PERFORMANCE_DATA(academicYearId)); // Note the extra 'data' nesting here from the controller.
  ...
  getFilterDictionary: async (): Promise<FilterDictionary> => {
    try {
      const response = await axiosInstance.get<ApiResponse<FilterDictionary>>(
