...
export async function fetchFacultyPerformanceForExport(
  ...
  try {
    ...
    console.error("Error fetching faculty performance data for export:", error);
    throw error;
  }
}
...
export function transformDataForExport(
  ...
  return apiData.faculties.map((faculty, index) => {
    ...
    const cleanName = faculty.Faculty_name.replace(/\s*\([^)]+\)$/, "").trim();
...
export function convertToCSV(data: FacultyPerformanceExportData[]): string {
  ...
  const csvContent = [
    ...
    ...data.map((row) =>
      headers
        .map((header) =>
          escapeCSVValue(row[header as keyof FacultyPerformanceExportData]),
        )
        .join(","),
    ),
  ].join("\n");
...
export async function exportFacultyPerformanceData(
  ...
  try {
    ...
    console.log(`Successfully exported ${exportData.length} faculty records`);
  } catch (error) {
